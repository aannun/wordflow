...
/// Handles the "books" folder where the user copies book files to read,
/// and scans it for available books. Mobile/desktop implementation.
class BookRepository {
  static Directory? _cachedDir;

  /// Returns the folder the user should copy books into, creating it if
  ...
  /// Lists every .txt/.pdf file found in the books folder, plus the books
  ...
  static Future<List<Book>> listBooks() async {
    ...
    final ownBooks = entries
        .whereType<File>()
        .where((f) => isSupportedBookFilename(f.path))
        .map(
          (f) => Book(
            id: f.path,
            title: titleFromFilename(f.path),
            loadWords: () async {
              final text = textFromBookBytes(await f.readAsBytes(), f.path);
